(function() {
    ...
    /**
     * Плагін редиректу для Lampa
     * Автор оригінального коду: Hlushok
     * Джерело: https://hlushok.github.io/lampa-plugin/redirect.js
     * Модифікації: додано сервер Syaivo та англійську назву
     */

    if (window.plugin_my_redirect_ready) return;
    window.plugin_my_redirect_ready = true;
    ...
    function isAlreadyOnTargetServer(targetUrl) {
        try {
            var currentHost = window.location.host;
            var targetHost = new URL(targetUrl).host;
    ...
    function startPlugin() {
        try {
            $('#REDIRECT').remove();
            var domainBUTT = '<div id="REDIRECT" class="head__action selector redirect-screen" title="Змінити сервер">' + icon_server_redirect + '</div>';
            ...
            $('#REDIRECT').on('hover:enter hover:click hover:touch', function() {
                var targetServer = Lampa.Storage.get('location_server');
                if (targetServer && !isAlreadyOnTargetServer(targetServer)) {
                    Lampa.Noty.show('Перехід на обраний сервер...');
            ...
            Lampa.SettingsApi.addParam({
                ...
                param: {
                    ...
                    values: {
                        ...
                        'http://siaivo.isroot.in': 'Syaivo'
                    },
                    default: 'http://lampaua.mooo.com/'
                },
                field: {
                    name: 'Server URL',
                    description: 'Виберіть сервер. Кнопка зʼявиться у верхньому барі.'
                },
            ...
            console.log('Помилка ініціалізації плагіна:', e);
        }
    }
