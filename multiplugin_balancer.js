...
function injectCSS() {
    ...
    style.innerHTML = `
        ...
        .multi-apply { text-align:center; margin-top:20px; padding:15px; border-radius:10px; font-weight:bold; background:#156DD1; color:#fff; cursor:pointer; display:none; }
        .multi-btn { padding:6px 12px; border-radius:6px; color:#fff; font-size:12px; }
        ...
        .modal-input { background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:#fff; padding:10px; border-radius:5px; margin-bottom:10px; width:100%; }
    `;
    ...
    var formHtml = $(`
        <div style="padding:20px;">
            ...
            <div style="display:flex; gap:10px;">
                <div class="selector" style="flex:1; background:#156DD1; text-align:center; padding:10px; border-radius:5px; cursor:pointer;">Зберегти</div>
                <div class="selector" style="flex:1; background:#555; text-align:center; padding:10px; border-radius:5px; cursor:pointer;">Скасувати</div>
            </div>
        </div>
    `);
...
function openAddModal(callback) {
    var formHtml = $(`
        <div style="padding:20px;">
            ...
            <input type="text" class="modal-input add-url" placeholder="URL скрипту">
            <div style="display:flex; gap:10px;">
                <div class="selector" style="flex:1; background:#46b85a; text-align:center; padding:10px; border-radius:5px; cursor:pointer;">Додати</div>
                <div class="selector" style="flex:1; background:#555; text-align:center; padding:10px; border-radius:5px; cursor:pointer;">Скасувати</div>
            </div>
        </div>
    `);
            ...
            var item = $(`
                <div class="multi-item selector" data-index="${index}">
                    <div>
                        ...
                        <span style="font-size:12px; color:rgba(255,255,255,0.6);">${src.url}</span>
                    </div>
                    <div style="display:flex; gap:10px; align-items:center;">
                        <div class="multi-toggle selector ${current ? 'enabled' : 'disabled'}" data-key="${key}">
