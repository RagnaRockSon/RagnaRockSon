...
function injectCSS() {
    ...
    style.innerHTML = `
        .multi-container { padding:20px; display:flex; flex-direction:column; gap:10px; }
        .multi-item { display:flex; justify-content:space-between; align-items:center; padding:15px; background:rgba(255,255,255,0.05); border-radius:10px; transition: all 0.3s ease; flex-wrap:wrap; }
        .multi-item .title { font-size:16px; font-weight:bold; flex:1; min-width:120px; }
        ...
        .multi-btn { padding:6px 12px; border-radius:6px; color:#fff; cursor:pointer; transition: all 0.3s ease; font-size:14px; }
        .multi-btn-edit { background:#FF9800; }
        .multi-btn-delete { background:#d24a4a; }
        .multi-btn-add { background:#46b85a; text-align:center; padding:12px; font-weight:bold; }
...
function openEditModal(index, callback) {
    ...
    var formHtml = $(`
        <div style="padding:20px;">
            <div style="margin-bottom:10px;"><label>Назва:</label><input type="text" class="modal-input edit-name" value="${src.name}" placeholder="Введіть назву"></div>
            <div style="margin-bottom:10px;"><label>URL:</label><input type="text" class="modal-input edit-url" value="${src.url}" placeholder="Введіть URL"></div>
...
function openAddModal(callback) {
    var formHtml = $(`
        <div style="padding:20px;">
            <div style="margin-bottom:10px;"><label>Назва:</label><input type="text" class="modal-input add-name" placeholder="Введіть назву джерела"></div>
            <div style="margin-bottom:10px;"><label>URL:</label><input type="text" class="modal-input add-url" placeholder="Введіть URL до скрипту"></div>
            <div style="display:flex; gap:10px;">
            ...
            var item = $(`
                <div class="multi-item selector">
                    ...
                    <div class="multi-actions">
                        <div class="multi-toggle ${current?'enabled':'disabled'}" data-key="${key}">${current?'Увімкнено':'Вимкнено'}</div>
