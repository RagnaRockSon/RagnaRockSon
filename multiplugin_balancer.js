(function () {
    ...
    function openEditModal(index, callback) {
        ...
        saveBtn.on('hover:enter', function () {
            ...
            tempState['multi_' + newName] = Lampa.Storage.get('multi_' + newName, false);
            hasChanges = true;
            Lampa.Modal.close();
    ...
    function openAddModal(callback) {
        ...
        addBtn.on('hover:enter', function () {
            ...
            var key = 'multi_' + newName;
            tempState[key] = false; // додаємо новий toggle у tempState
        ...
        function renderSources() {
            ...
            sources.forEach(function (src, index) {
                ...
                var item = $(`
                    <div class="multi-item selector" data-index="${index}">
                        ...
                        <div style="display:flex; gap:10px; align-items:center;">
                            <div class="multi-toggle ${tempState[key] ? 'enabled' : 'disabled'}" data-key="${key}">
                ...
                item.find('.multi-btn-delete').on('hover:enter', function () {
                    ...
                    var keyToDelete = 'multi_' + sources[idx].name;
                    sources.splice(idx, 1);
                    delete tempState[keyToDelete];
                    saveSourcesToStorage();
                    hasChanges = true;
                    renderSources();
                    updateTitle($('.modal__title'));
